...
(1, 'Tiffany Knot Ring', 'Tiffany & Co.', 'https://media.tiffany.com/is/image/Tiffany/EcomItemL2/tiffany-knotring-68886364_1020084_ED_M.jpg', 'Nhẫn White Gold đính kim cương với thiết kế nút thắt tượng trưng cho sự gắn kết vĩnh cửu.', 'Khơi nguồn cảm hứng từ kiến trúc và nhịp sống của thành phố New York, bộ sưu tập Tiffany Knot đại diện cho những sợi dây gắn kết không thể đứt lìa trong cuộc sống của chúng ta.', 1, 1),
(2, 'Tiffany Lock Bangle', 'Tiffany & Co.', 'https://media.tiffany.com/is/image/Tiffany/EcomItemL2/tiffany-lockbangle-70180422_1052959_ED.jpg', 'Lắc tay Vàng hồng 18k mang mãnh lực tình yêu.', 'Biểu tượng của những mối liên kết tạo nên con người chúng ta, Tiffany Lock tựa như một biểu tượng hiện đại của tình yêu trọn đời. Lấy cảm hứng từ hoạ tiết ổ khoá kinh điển từ thập niên 1880.', 3, 1),
(3, 'Tiffany T Smile Pendant', 'Tiffany & Co.', 'https://media.tiffany.com/is/image/Tiffany/EcomItemL2/tiffany-t-smilependant-62617659_997784_ED_M.jpg', 'Dây chuyền vàng mang nụ cười tinh tế.', 'Bộ sưu tập Tiffany T mang đậm dấu ấn kiến trúc với đường nét thanh mảnh, sắc nét. Hình mặt cười tượng trưng cho niềm vui và sự lạc quan không bao giờ vụt tắt.', 2, 2),
(4, 'Elsa Peretti® Diamonds by the Yard Earrings', 'Tiffany & Co.', 'https://media.tiffany.com/is/image/Tiffany/EcomItemL2/elsa-perettidiamonds-by-the-yardearrings-12818653_936173_ED.jpg', 'Bông tai kim cương mang vẻ đẹp rực rỡ và thuần khiết.', 'Nhà thiết kế huyền thoại Elsa Peretti đã thực hiện một cuộc cách mạng trong lĩnh vực trang sức, biến viên kim cương quý giá trở nên gần gũi với phong cách hằng ngày.', 4, 1);
...
ALTER TABLE `order_items` ADD PRIMARY KEY (`id`), ADD KEY `fk_oi_order` (`order_id`), ADD KEY `fk_oi_variant` (`variant_id`);
...
ALTER TABLE `wishlist` ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `unique_wishlist_variant` (`user_id`,`variant_id`), ADD KEY `fk_wishlist_variant` (`variant_id`);
...
ALTER TABLE `order_items` ADD CONSTRAINT `fk_oi_order` FOREIGN KEY (`order_id`) REFERENCES `orders` (`id`) ON DELETE CASCADE, ADD CONSTRAINT `fk_oi_variant` FOREIGN KEY (`variant_id`) REFERENCES `product_variants` (`id`);
...
ALTER TABLE `wishlist` ADD CONSTRAINT `fk_wishlist_user_new` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE, ADD CONSTRAINT `fk_wishlist_variant` FOREIGN KEY (`variant_id`) REFERENCES `product_variants` (`id`) ON DELETE CASCADE;
